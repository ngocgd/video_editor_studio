...
- **Downloader**: resumable Range downloads into `.staging/<sha>.part`. The sha256 covers the whole file. A host allowlist (huggingface.co and hf.co, https only) also applies to redirects. Oversized bodies are cut off. Before downloading, a disk pre-flight checks that free space is at least what remains to download plus 40 GB of headroom, on both the models volume and the host drive that holds Docker's data disk (read through a read-only bind mount at `/host-disk`). It uses the smaller of the two figures and refuses the download when the host figure cannot be read. Files already on disk are adopted after hashing instead of being downloaded again.
...
## Review round 1 (reviewer report `plans/reports/code-reviewer-260926-1007-phase-09a-comfyui-image-engines-review.md`)

| Item | Outcome |
|---|---|
| **H1** (High, blocking): the disk pre-flight measured the Docker VM's virtual disk, not the host disk | **Fixed** in `e72e1ca` and `46613f8`. Measured first: inside a container, statfs on a fresh volume reports about 800 GB free (the maximum size of the virtual disk). statfs on a read-only bind mount of a host directory reports the real `C:` drive: `df -k` showed 100,107,752 KB available, matching the host's 96 GB free. `Downloader.HostDiskDir` is therefore a host bind mount. The pre-flight passes only when `min(host free, volume free) >= remaining download + 40 GB headroom`. Each downloaded byte grows the virtual disk by at most one byte, so the whole remaining download is charged against the host drive. The figure is read live on every pull, so it cannot go stale. The pre-flight refuses (`ErrHostDiskUnknown`, a permanent failure) when the directory is unset or unreadable, so a worker missing the mount can never download. `compose.gpu.yml` mounts `${MODELS_HOST_DISK_SOURCE:-./host-disk}` read-only at `/host-disk` in the worker and the `cli` service and sets `MODELS_HOST_DISK_DIR`. `.env.example` documents the source variable, which must point at the drive that holds Docker's data disk. New tests: `TestInstallDiskPreflightIsBoundedByTheHostDisk` (800 GiB volume with 8999 host bytes is refused, a full volume with a roomy host is refused, and exactly enough host space passes) and `TestInstallDiskPreflightRefusesWithoutAHostDiskFigure` (unset directory, and an unmounted directory under real statfs; neither sends a request). The first test fails against the old code, which read only the volume |
| M1–M3, L1–L4 (not blocking) | Open. Not in this fix pass's scope; they remain as the reviewer recorded them |
| L5 (the report said the hardened image was unbuilt) | Noted. `loomtale/comfyui:local` exists from an earlier build. The **current** Dockerfile (with the scratch `mkdir` entrypoint) is unbuilt, because rebuilding needs network downloads that are paused |

Re-run after the fix: `scripts/tb.sh gen lint test` exit 0 (golangci-lint 0 issues, manifest lint OK, all Go packages ok with `-race`, pytest 26 passed). `git status` was clean after `gen`, so there is no generated-code drift. The three pre-flight tests pass verbosely. `docker compose -f compose.yml -f compose.gpu.yml --profile cli config` shows the `/host-disk` bind and `MODELS_HOST_DISK_DIR` on the worker and the cli. I did not re-run the integration suite or the e2e: neither uses the GPU overlay or reaches the pre-flight, and nothing they exercise changed. A live pull through the pre-flight is **pending: model downloads paused by the user**.
